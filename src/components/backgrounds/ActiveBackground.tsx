...
export function ActiveBackground({ theme = 'dark' }: ActiveBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const drawHeart = (h: Heart) => {
      ...
      const pulse = 1 + Math.sin(h.pulsePhase) * 0.15;
      ...
      ctx.bezierCurveTo(-size * 0.6, -size * 0.2, -size * 0.6, size * 0.3, 0, size * 0.7);
      ctx.bezierCurveTo(size * 0.6, size * 0.3, size * 0.6, -size * 0.2, 0, size * 0.3);
      ctx.fill();
    ...
    const drawStar = (s: Star) => {
      ...
      ctx.fillStyle = effectiveTheme === 'dark' ? '#38BDF8' : '#0ea5e9';
    ...
    const drawCircle = (c: FloatingCircle) => {
      ...
      const gradient = ctx.createRadialGradient(c.x, c.y, 0, c.x, c.y, currentSize);
    ...
    const drawHalftone = (ht: Halftone) => {
      ...
      ctx.fillStyle = effectiveTheme === 'dark' ? 'rgba(56, 189, 248, 0.3)' : 'rgba(14, 165, 233, 0.2)';
    ...
    const drawSpeedLines = () => {
      ...
      if (Math.floor(timeRef.current) % 5 !== 0) return;
    ...
    const animate = (timestamp: number) => {
      ...
      circlesRef.current.forEach((c) => {
        ...
        if (c.y < -c.size - 30) Object.assign(c, createCircle());
        drawCircle(c);
      });
      ...
      heartsRef.current.forEach((h) => {
        ...
        h.pulsePhase += 0.05;
        if (h.y < -h.size - 50) Object.assign(h, createHeart());
        drawHeart(h);
      });
