...
interface SecurityAlertRequest {
    ...
    ip_address?: string; // Now receives a SHA-256 hash from the client
    browser?: string;
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        // The ip_address field now contains a SHA-256 hash from the client.
        // Store it as-is — no raw IP ever reaches the database.
        ...
        // Show truncated IP hash in Telegram alert (never raw IP)
        const ipDisplay = body.ip_address ? `${body.ip_address.slice(0, 16)}…` : 'Unknown';
...
<b>IP Hash:</b> <code>${sanitize(ipDisplay)}</code>
